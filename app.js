...
const updateValor = () => { valor.value = (Number(qtdBordos.value || 0) * 5).toFixed(2); };
qtdBordos.addEventListener('input', updateValor);
prefixo.addEventListener('input', () => { prefixo.value = prefixo.value.replace(/\D/g,'').slice(0,3); });
dataCaixa.value = todayISO();

// ---- Auth flows ----
btnRegistrar.addEventListener('click', async () => {
  const nome = cadNome.value.trim(), mat = cadMatricula.value.trim(), senha = cadSenha.value;
  ...
  try {
    ...
    await setDoc(doc(db, 'users', cred.user.uid), { nome, matricula: mat, admin: isAdmin, createdAt: serverTimestamp() });
    alert('Conta criada! Faça login com sua matrícula e senha.');
    cadNome.value = cadMatricula.value = cadSenha.value = '';
    loginMatricula.value = mat; loginSenha.value = '';
    window.scrollTo({ top: 0, behavior: 'smooth' });
  } catch(e) { alert('Erro ao criar conta: '+(e?.message||e)); }
});

btnLogin.addEventListener('click', async () => {
  const mat = loginMatricula.value.trim(), senha = loginSenha.value;
  if (!mat || !senha) return alert('Informe matrícula e senha.');
  try { await signInWithEmailAndPassword(auth, emailFromMat(mat), senha); }
...
onAuthStateChanged(auth, async (user)=>{
  if(!user){ authArea.classList.remove('hidden'); appArea.classList.add('hidden'); userBadge.classList.add('hidden');
    btnLogout.classList.add('hidden'); btnChangePass.classList.add('hidden'); currentUserDoc=currentCaixaRef=null; return;
  }
  const uref = doc(db,'users',user.uid);
  const snap = await getDoc(uref);
  currentUserDoc = snap.data();
  if(adminsMat.has(currentUserDoc?.matricula) && !currentUserDoc.admin){ await updateDoc(uref,{admin:true}); currentUserDoc.admin=true; }

  authArea.classList.add('hidden'); appArea.classList.remove('hidden');
  btnLogout.classList.remove('hidden'); btnChangePass.classList.remove('hidden');
...
function enableWorkflows(aberto){
  btnAbrir.disabled=!!aberto; btnFechar.disabled=!aberto;
  lancBox.classList.toggle('hidden',!aberto); sangriaBox.classList.toggle('hidden',!aberto);
}
...
btnAbrir.addEventListener('click', async ()=>{
  ...
  const caixa={ status:'aberto', createdAt:serverTimestamp(), data:dataCaixa.value, matricula:currentUserDoc.matricula, nome:currentUserDoc.nome };
...
$('#btnRegistrarSangria').addEventListener('click', async ()=>{
  if(!currentCaixaRef) return alert('Abra um caixa primeiro.');
  const valorS=Number($('#sangriaValor').value||0), motivo=($('#sangriaMotivo').value||'').trim();
  ...
  $('#sangriaValor').value=''; $('#sangriaMotivo').value=''; await renderParcial();
  alert('Sangria registrada.');
});

// ---- Relatório parcial com horário ----
async function renderParcial(){
  ...
  let html=`<h3>Usuário: ${currentUserDoc.nome} • Matrícula: ${currentUserDoc.matricula}</h3>`;
  html+=`<h4>LANÇAMENTOS:</h4>`;
  html+=`<table border="1" cellpadding="4" cellspacing="0" style="border-collapse: collapse; width: 100%;">
    <thead>
      <tr style="background-color:#ccc;">
        <th>Horário</th><th>Data Caixa</th><th>Prefixo</th><th>Validador</th>
        <th>Qtd Bordos</th><th>Valor</th><th>Motorista</th>
      </tr>
    </thead><tbody>`;
  ...
  lqs.forEach(d=>{
    ...
    const horaLanc=x.createdAt?.toDate? x.createdAt.toDate().toLocaleTimeString('pt-BR'):'';
    html+=`<tr>
      <td style="text-align:center;">${horaLanc}</td>
      <td style="text-align:center;">${formatISOtoBR(x.dataCaixa)}</td>
      <td style="text-align:center;">${x.prefixo||''}</td>
      <td style="text-align:center;">${x.tipoValidador||''}</td>
      <td style="text-align:center;">${x.qtdBordos||''}</td>
      <td style="text-align:right;">${fmtMoney(x.valor)}</td>
      <td style="text-align:center;">${x.matriculaMotorista||''}</td>
    </tr>`;
  });
  ...
  html+=`<h4>SANGRIAS:</h4>`;
  html+=`<table border="1" cellpadding="4" cellspacing="0" style="border-collapse: collapse; width: 100%;">
    <thead><tr style="background-color:#ccc;"><th>Valor</th><th>Motivo</th></tr></thead><tbody>`;

  if(!sqs.empty){ sqs.forEach(d=>{ const x=d.data(); totalS+=Number(x.valor||0);
    html+=`<tr><td style="text-align:right;">${fmtMoney(x.valor)}</td><td>${x.motivo||''}</td></tr>`; });
  } else { html+=`<tr><td colspan="2" style="text-align:center;">— Nenhuma —</td></tr>`; }

  html+=`</tbody></table>`;
  html+=`<h4>Total Lançamentos: ${fmtMoney(total)}</h4>`;
  html+=`<h4>Total Sangrias: ${fmtMoney(totalS)}</h4>`;
  html+=`<h4>Total Corrigido: ${fmtMoney(total-totalS)}</h4>`;

  relatorioLista.innerHTML=html;
}

// ---- Recibo térmico ----
function printThermalReceipt(data){
  const win=window.open('','_blank','width=400,height=800');
  const now=new Date(), dt=now.toLocaleString('pt-BR'), dataCaixaBR=formatISOtoBR(data.dataCaixa);
  const html=`<!DOCTYPE html>
  <html><head><meta charset="utf-8"><title>Recibo</title>
  <style>@page{size:80mm 148mm;margin:0;}body{font-family:"Lucida Sans",monospace;font-size:12px;margin:0;padding:0;}
  h1{text-align:center;font-size:15px;margin:8px 0 12px;}.mono{font-family:"Lucida Sans",monospace;white-space:pre-wrap;}</style></head>
...
--------------------------------------------------------------------
  ...
  <strong>Assinatura Recebedor:</strong><br>______________________________
  </div></body></html>`;
...
async function gerarRelatorioPDF(){
  ...
  const uid=currentCaixaRef.userId, cid=currentCaixaRef.caixaId;

  const logo=new Image(); logo.src="./assets/logo.png";
  logo.onload=async ()=>{
    const pageWidth=docpdf.internal.pageSize.getWidth(), logoWidth=120, logoHeight=60;
    docpdf.addImage(logo,'PNG',(pageWidth-logoWidth)/2,30,logoWidth,logoHeight);
    ...
    docpdf.text('Relatório de Fechamento de Caixa',pageWidth/2,y,{align:'center'}); y+=30;
    docpdf.setFontSize(11); docpdf.setFont('helvetica','normal');

    const hoje=new Date(), dataHoraBR=hoje.toLocaleDateString('pt-BR')+' '+hoje.toLocaleTimeString('pt-BR');
    const caixaSnap=await getDoc(doc(db,'users',uid,'caixas',cid)), caixaData=caixaSnap.data();
    let aberturaTxt="";
    if(caixaData?.data){
    ...
    const lqs=await getDocs(query(lref,orderBy('createdAt','asc')));
    const lancamentosBody=[], sref=collection(db,'users',uid,'caixas',cid,'sangrias');
    let total=0, totalS=0;
    lqs.forEach(d=>{
      const x=d.data(); total+=Number(x.valor||0);
      const horaLanc=x.createdAt?.toDate? x.createdAt.toDate().toLocaleTimeString("pt-BR"):'';
      lancamentosBody.push([`${horaLanc} ${formatISOtoBR(x.dataCaixa)}`, x.prefixo||'', x.tipoValidador||'', x.qtdBordos||'', fmtMoney(x.valor)||'R$ 0,00', x.matriculaMotorista||'']);
    });

    docpdf.autoTable({
      startY:y, head:[['Horário/Data','Prefixo','Validador','Qtd Bordos','Valor','Motorista']],
      body:lancamentosBody, theme:'grid',
    ...
    docpdf.autoTable({ startY:y, head:[['Valor','Motivo']], body:sangriasBody, theme:'grid', headStyles:{fillColor:[200,200,200]} });
    y=docpdf.lastAutoTable.finalY+10;
